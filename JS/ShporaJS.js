...
Performance.now()  //милисекунды, приминяется в анимации

var t0 = performance.now();  //зафиксировали время в милисекундах
doSomething();               //сделли чтонибудь
var t1 = performance.now();  //зафиксировали время после в милисекундах
console.log("Call to doSomething took " + (t1 - t0) + " milliseconds.")// разница их это время выполения

//////////////////requestAnimationFrame(step);/////////////////////////
requestAnimationFrame(step) // это улучшаный setTimeout, setTimeout четко привязанный ко времени и не
                            // обращает внимание на прозводительность браузера, а requestAnimationFrame берет это в учет
//приемр показавыющий разницу
...
step();
//мы не указали интервал. Как часто бдет вызываться наша функция? Все зависит от
// частоты кадров вашего браузера и компьютера (обычно это 60 кадров в секунду)
